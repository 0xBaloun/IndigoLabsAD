...
resource "local_file" "ansible-groupvars-elastic-creation" {
  ...
  depends_on = [
    random_string.linuxpass,
    azurerm_network_interface.indigolabs-vm-elastic-nic
  ]
  
  content = templatefile("${path.module}/../Ansible/group_vars/linux.tmpl",
           { 
              password = random_string.linuxpass.result
              ip       = azurerm_network_interface.indigolabs-vm-elastic-nic.private_ip_address
           })
...
resource "null_resource" "ansible-provisioning" {
  ...
  triggers = {
    ...
    winserv2019_id = azurerm_windows_virtual_machine.indigolabs-vm-winserv2019.id
    ...
    elastic_id = azurerm_linux_virtual_machine.indigolabs-vm-elastic.id
  }
