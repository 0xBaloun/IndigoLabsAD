# Network Interface
resource "azurerm_network_interface" "indigolabs-vm-windows10-nic" {
  name                 = "indigolabs-vm-windows10-nic"
  ...
  ip_configuration {
    name                          = "indigolabs-vm-windows10-config"
...
resource "azurerm_network_interface_nat_rule_association" "indigolabs-vm-windows10-nic-nat" {
  network_interface_id  = azurerm_network_interface.indigolabs-vm-windows10-nic.id
  ip_configuration_name = "indigolabs-vm-windows10-config"
  nat_rule_id           = azurerm_lb_nat_rule.indigolabs-lb-nat-rdp.id
}

# Virtual Machine
resource "azurerm_windows_virtual_machine" "indigolabs-vm-windows10" {
  name                = "indigolabs-vm-windows10"
  ...
  network_interface_ids = [
    azurerm_network_interface.indigolabs-vm-windows10-nic.id,
  ]

  os_disk {
    name                 = "indigolabs-vm-windows10-osdisk"
    caching              = "ReadWrite"
