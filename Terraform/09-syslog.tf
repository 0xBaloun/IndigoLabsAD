...
resource "azurerm_network_interface_nat_rule_association" "indigolabs-vm-syslog-nic-nat" {
  network_interface_id  = azurerm_network_interface.indigolabs-vm-syslog-nic.id
  ip_configuration_name = "indigolabs-vm-syslog-nic-config"
  nat_rule_id           = azurerm_lb_nat_rule.indigolabs-lb-nat-ssh.id
}

# Virtual Machine
resource "azurerm_syslog_virtual_machine" "indigolabs-vm-syslog" {
  ...
  admin_password      = random_string.syslogpass.result
